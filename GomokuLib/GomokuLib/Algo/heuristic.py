...
@njit()
def _find_align_reward(board, graph, sr, sc, player_idx, dirs, p):
    ...
    for y in range(2 + gz_start_r, 3 + gz_end_r):
        for x in range(2 + gz_start_c, 3 + gz_end_c):
            ...
            if board_pad[player_idx, y, x]:
                reward += (_find_align_reward(board_pad, my_h_graph, y, x, player_idx, dirs, p)
                    + my_cap_coef * _find_align_reward(board_pad, my_cap_graph, y, x, player_idx, dirs, p))

            elif board_pad[player_idx ^ 1, y, x]:
                reward += (_find_align_reward(board_pad, opp_h_graph, y, x, player_idx, dirs, p)
                    + opp_cap_coef * _find_align_reward(board_pad, opp_cap_graph, y, x, player_idx, dirs, p))
