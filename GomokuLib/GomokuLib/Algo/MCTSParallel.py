...
from GomokuLib.Algo.MCTSWorkerNoJit import MCTSWorkerNoJit as MCTSWorker
from GomokuLib.Algo.MCTSEvalLazy import MCTSEvalLazy
from GomokuLib.Game.GameEngine import Gomoku


@njit()
def force_worker(game_engine, worker):

    for i in range(10000):
        do_your_fck_work(worker, game_engine, 0, 0)


@njit(nogil=True)
def do_your_fck_work(worker: MCTSWorker, game_engine: Gomoku, pool_id: Typing.MCTSIntDtype, buff_id: Typing.MCTSIntDtype) -> tuple:
    return worker.do_your_fck_work(game_engine, pool_id, buff_id)
...
class MCTSParallel:
    ...
    def __call__(self, game_engine: Gomoku) -> tuple:
        ...
        force_worker(game_engine, self.workers[0])
    ...
    def work(self):
        ...
        while _iter < self.mcts_iter:
            ...
            print(f"mcts iter: {_iter}")
                # if _iter < self.mcts_iter:
                #     break

        # next_f = set()   # Set() object
        # nn = 0
        # next_f = []
    ...
    def submit_worker(self, worker_id: int) -> concurrent.futures:
        ...
        worker_f = self.pool.submit(do_your_fck_work, self.workers[worker_id], self.game_engine, self.pool_id, self.buff_id)
