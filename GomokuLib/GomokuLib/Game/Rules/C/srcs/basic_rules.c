...
int is_winning(char *board, int ar, int ac, int gz_start_r, int gz_start_c, int gz_end_r, int gz_end_c)
{
...
//        int     no_edge = 0 <= r2 && r2 < rmax && 0 <= c2 && c2 < cmax;
        ...
        for (int i = 0; i < 4 && count1 == 4; ++i)  // Check 4 previous cells
        {
            ...
            if (no_edge)
            {
                if (board[r1 * cmax + c1] == 0)     // Stop if no stone found
                    count1 = i;                     // Save stone align length (Default at 4)
            }
//            else if (r1 < 0 || r1 >= rmax || c1 < 0 || c1 >= cmax || board[r1 * cmax + c1] == 0)
            else if (r1 < gz_start_r || gz_end_r < r1 || c1 < gz_start_c || gz_end_c < c1 || board[r1 * cmax + c1] == 0)
                count1 = i;
        }
