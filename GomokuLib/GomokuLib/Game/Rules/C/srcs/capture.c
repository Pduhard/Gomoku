...
int count_captures(char *board, int ar, int ac, int gz_start_r, int gz_start_c, int gz_end_r, int gz_end_c, int player_idx)
{
    int         count = 0;
    static int  cmax = 19;
...
//        if (0 <= r1 && r1 < rmax && 0 <= c1 && c1 < cmax && board[r1 * cmax + c1] == 1)                       // Second stone that made capture
        ...
        {
            int opp_i1 = opp_idx + (r1 - dr) * cmax + (c1 - dc);            // Second captured stone
//            fprintf(stderr, "La boucle est semi pleine ! %d %d: opp %d %d -> opp %d / %d\n", ar, ac, r1 - dr, c1 - dc, board[opp_i1], board[opp_i1 - 361]);
            if (board[opp_i1] == 1)
            {
                int opp_i0 = opp_idx + (ar + dr) * cmax + (ac + dc);        // First captured stone
